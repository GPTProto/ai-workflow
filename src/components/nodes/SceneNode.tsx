...
import { Image, Loader2, AlertCircle, ZoomIn, RefreshCw, X, Check, Upload } from 'lucide-react';
import { cn } from '@/lib/utils';
import { IMAGE_EDIT_MODELS, getDefaultImageModel, getDefaultAspectRatio, getDefaultImageSize } from '@/config/api';
import type { ImageNodeData, NodeStatus, ImageEditModelId } from '@/types/workflow';
...
const statusConfig: Record<NodeStatus, {
  ...
  running: {
    border: 'border-[#22d3ee]/70',
    bg: 'bg-[#164e63]',
    glow: 'glow-cyan animate-pulse-glow',
    text: 'text-[#22d3ee]',
    handleColor: '#22d3ee',
  },
  success: {
...
function SceneNode({ data, id }: NodeProps<ImageNodeData>) {
  const { label, status, imageUrl, prompt, error, onPreview, onRegenerate, onEditPrompt, onUpload, onRegenerateWithModel } = data;
  ...
  const [selectedModel, setSelectedModel] = useState<ImageEditModelId>(() => {
    const defaultImageModel = getDefaultImageModel();
    const modelMap: Record<string, ImageEditModelId> = {
      'gemini': 'gemini-edit',
      'seedream': 'seedream-edit',
      'wan-t2i': 'wan-edit',
    };
    return modelMap[defaultImageModel] || 'gemini-edit';
  });
  // Initialize size based on the selected model's default, not global default
  const [selectedSize, setSelectedSize] = useState<string>(() => {
    ...
    const editModelId = modelMap[defaultImageModel] || 'gemini-edit';
    return IMAGE_EDIT_MODELS[editModelId].defaultSize;
  });
  const [selectedAspectRatio, setSelectedAspectRatio] = useState<string>(() => {
    const defaultImageModel = getDefaultImageModel();
    if (defaultImageModel === 'gemini') {
  ...
  useEffect(() => {
    const model = IMAGE_EDIT_MODELS[selectedModel];
  ...
  const handleRegenerateClick = (e: React.MouseEvent) => {
    ...
    if (onRegenerateWithModel) {
      ...
      const editModelId = modelMap[defaultImageModel] || 'gemini-edit';
      setSelectedModel(editModelId);
      // Use global default size, or fall back to model's default size
      const globalSize = getDefaultImageSize();
      const modelSizes = IMAGE_EDIT_MODELS[editModelId].sizeOptions.map(s => s.value);
      if (modelSizes.includes(globalSize)) {
        setSelectedSize(globalSize);
      } else {
        setSelectedSize(IMAGE_EDIT_MODELS[editModelId].defaultSize);
      }
  ...
  const handleConfirmRegenerate = (e: React.MouseEvent) => {
    e.stopPropagation();
    const currentModel = IMAGE_EDIT_MODELS[selectedModel];
    ...
    console.log('[SceneNode] handleConfirmRegenerate called', {
      selectedModel,
      sizeToPass,
      aspectRatioToPass,
      editedPrompt,
      hasOnRegenerateWithModel: !!onRegenerateWithModel,
      hasOnEditPrompt: !!onEditPrompt,
    });

    // First update prompt if changed
    if (onEditPrompt && editedPrompt.trim() && editedPrompt.trim() !== prompt) {
      console.log('[SceneNode] Updating prompt before regenerate...');
      onEditPrompt(editedPrompt.trim());
    }

    if (onRegenerateWithModel) {
      console.log('[SceneNode] Calling onRegenerateWithModel...');
      onRegenerateWithModel(selectedModel, sizeToPass, aspectRatioToPass);
    } else {
      console.warn('[SceneNode] onRegenerateWithModel is undefined!');
      setIsRegenerateMode(false);
    }
  };
  ...
  if (isRegenerateMode) {
    ...
    const modelIconMap: Record<string, string> = {
      'gemini-edit': '/gemini.png',
      'seedream-edit': '/seedance.png',
      'wan-edit': '/wan.png',
    };
        ...
        <div className="p-3">
          ...
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {Object.values(IMAGE_EDIT_MODELS).map((model) => (
                <button
                  key={model.id}
                ...
                >
                  ...
                  <span>{model.name.replace(' Edit', '')}</span>
                  {selectedModel === model.id && (
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button
              type="button"
              onClick={(e) => {
                console.log('[SceneNode] Generate button clicked');
  ...
  return (
    <div
      className={cn(
        'rounded-2xl border-2 overflow-hidden w-[180px]',
    ...
    >
      ...
      {/* Image preview area - 9:16 ratio */}
      <div className="relative w-full h-[95px] bg-[#1a1a1a] flex items-center justify-center overflow-hidden group">
        {status === 'success' && imageUrl ? (
              ...
              >
                <div className="bg-slate-900/95 border border-slate-600/50 rounded-xl p-2 shadow-2xl backdrop-blur-sm">
                  <img
                    src={imageUrl}
                    alt={label}
                    className="max-w-[320px] max-h-[320px] object-contain rounded-lg"
                  />
                  <div className="mt-1.5 text-xs text-slate-400 text-center truncate max-w-[320px]">
                    {label}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-center gap-1.5 p-2">
                ...
                {(onRegenerateWithModel || onRegenerate) && (
                  <button
                    ...
                    className="w-6 h-6 rounded-lg bg-[#22d3ee]/60 backdrop-blur-sm flex items-center justify-center hover:bg-[#22d3ee]/80 transition-colors"
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="relative">
              ...
              <Loader2 className="w-5 h-5 text-[#22d3ee] absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
            </div>
            <span className="text-xs text-[#22d3ee] font-medium">Generating...</span>
          </div>
        ) : status === 'error' ? (
          <div className="flex flex-col items-center gap-1 px-2">
            ...
            <div className="flex gap-1 justify-center">
              ...
              {(onRegenerateWithModel || onRegenerate) && (
                <button
                  onClick={handleRegenerateClick}
                  className="px-1.5 py-0.5 rounded bg-[#22d3ee]/80 hover:bg-[#22d3ee] text-white text-[10px] transition-all flex items-center gap-0.5"
                >
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="w-10 h-10 rounded-xl bg-[#2a2a2a] border border-[#4A90E2]/30 flex items-center justify-center">
              <Image className="w-5 h-5 text-[#888888]" />
            </div>
...
function arePropsEqual(prevProps: NodeProps<ImageNodeData>, nextProps: NodeProps<ImageNodeData>) {
  return (
...
export default memo(SceneNode, arePropsEqual);
